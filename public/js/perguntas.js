...
function aleatorizarPerguntas () {
    listaDeQuestoes = []
  for (let i = 0; i < 8; i++) {
    let numeroDaPergunta = Math.floor(Math.random() * 49 + 1);
    let perguntaAtual = listaDePerguntas[numeroDaPergunta]; // listaDePerguntas
    if(!(listaDeQuestoes.includes(perguntaAtual))){
      listaDeQuestoes.push(perguntaAtual)
    }
  }
  quantidadeDePerguntas = listaDeQuestoes.length
}

const listaDePerguntas = [ // listaDePerguntas
  {
  ...
  {
    pergunta:
      "Qual personagem fictício aparece em múltiplos projetos de Tyler?",
  ...
  {
    pergunta:
      "Tyler participou de qual festival como headliner pela primeira vez?",
